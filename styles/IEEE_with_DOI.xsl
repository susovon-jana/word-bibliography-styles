...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Start">
      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1033'">
            <xsl:text>IEEE with DOI</xsl:text>
          </xsl:when>
      ...
      <xsl:when test="b:category">
				<xsl:text>IEEE with DOI (2026)</xsl:text>
			</xsl:when>
...
<xsl:call-template name = "BibDisplayDOI"/>
	...
	<xsl:template name="BibDisplayDOI">
		<xsl:variable name="doi">
			<xsl:choose>
				<xsl:when test="b:DOI and normalize-space(b:DOI) != ''">
					<xsl:value-of select="normalize-space(b:DOI)"/>
				</xsl:when>
				<xsl:when test="starts-with(normalize-space(b:URL), '10.')">
					<xsl:value-of select="normalize-space(b:URL)"/>
				</xsl:when>
				<xsl:when test="starts-with(normalize-space(b:Comments), 'DOI:')">
					<xsl:value-of select="normalize-space(substring-after(normalize-space(b:Comments), 'DOI:'))"/>
				</xsl:when>
				<xsl:when test="starts-with(normalize-space(b:Comments), '10.')">
					<xsl:value-of select="normalize-space(b:Comments)"/>
				</xsl:when>
			</xsl:choose>
		</xsl:variable>
		<xsl:choose>
			<xsl:when test="normalize-space($doi) != ''">
				...
				<a>
					<xsl:attribute name="href">
						<xsl:value-of select="concat('https://doi.org/', normalize-space($doi))"/>
					</xsl:attribute>
					<xsl:value-of select="normalize-space($doi)"/>
				</a>
			</xsl:when>
			<xsl:when test="contains(normalize-space(b:URL), 'doi.org/')">
				<xsl:text> </xsl:text>
				<a>
					<xsl:attribute name="href">
						<xsl:value-of select="normalize-space(b:URL)"/>
			...
			<xsl:when test="b:URL and normalize-space(b:URL) != ''">
				<xsl:text> Available: </xsl:text>
				<a>
					<xsl:attribute name="href">
						<xsl:value-of select="normalize-space(b:URL)"/>
