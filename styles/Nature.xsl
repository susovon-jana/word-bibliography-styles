  ...
  <xsl:template match="/">
    ...
    <xsl:choose>
      ...
      <xsl:when test="b:OfficeStyleKey">
        <xsl:text>NatureJournal</xsl:text>
      </xsl:when>
                    ...
                    <xsl:choose>
                      <xsl:when test="b:SourceType='Book'">
                        ...
                        <xsl:variable name="sg3">
                          ...
                          <xsl:if test="string-length(b:Publisher)>0 or string-length($year)>0">
                            <xsl:if test="string-length(normalize-space($sg2))>0">
                              <xsl:text> </xsl:text>
                            </xsl:if>
                            <xsl:value-of select="b:Publisher"/><xsl:if test="string-length($year)>0"><xsl:text>, </xsl:text><xsl:value-of select="$year"/></xsl:if>
                          </xsl:if>
                        </xsl:variable>
                      ...
                      <xsl:when test="b:SourceType='BookSection'">
                        ...
                        <xsl:variable name="sg4">
                          ...
                          <xsl:if test="string-length($editorLF)>0">
                            ...
                            <xsl:call-template name="secBy"><xsl:with-param name="role" select="&quot;Editor&quot;"/></xsl:call-template><xsl:choose><xsl:when test="$cEditors=1"><xsl:text> (ed.)</xsl:text></xsl:when><xsl:otherwise><xsl:text> (eds.)</xsl:text></xsl:otherwise></xsl:choose>
                          </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="sg5">
                          <xsl:copy-of select="$sg4"/>
                        ...
                        <xsl:variable name="sg6">
                          ...
                          <xsl:if test="string-length(b:Pages)>0">
                            ...
                            <xsl:text>pp </xsl:text><xsl:value-of select="b:Pages"/>
                          </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="rf">
                          <xsl:if test="string-length($doi)=0">
