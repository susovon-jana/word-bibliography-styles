...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"	xmlns:b="http://schemas.openxmlformats.org/officeDocument/2006/bibliography" xmlns:t="http://www.microsoft.com/temp">
  ...
  <xsl:template match="/">
    ...
    <xsl:choose>
      ...
      <xsl:when test="b:OfficeStyleKey">
        <xsl:text>HarvardReferencing</xsl:text>
      </xsl:when>
                                                ...
                                                <xsl:variable name="author0">
                                                        <xsl:choose>
                                                				...
                                                				<xsl:call-template name="formatNameCore">
                                                				        <xsl:with-param name="FML" select="'%L'"/>
                                                				        <xsl:with-param name="FM" select="'%L'"/>
                                                				        <xsl:with-param name="ML" select="'%L'"/>
                                                				        <xsl:with-param name="FL" select="'%L'"/>
                                                        ...
                                                        <xsl:variable name="yearDisplay">
                                                                <xsl:choose>
                                                                        <xsl:when test="msxsl:node-set($ListPopulatedWithMain)/b:Citation/b:NoYear">
                                                                        </xsl:when>
                                                                        <xsl:when test="string-length($year0)=0 and string-length($lead)>0">
                                                                                <xsl:text>n.d.</xsl:text>
                                                                        </xsl:when>
                                                                        <xsl:otherwise>
                                                        ...
                                                        <xsl:if test="string-length($yearDisplay)>0">
                                                                <xsl:if test="string-length($lead)>0">
                                                                        <xsl:text> </xsl:text>
                                                                </xsl:if>
                                                                <xsl:value-of select="$yearDisplay"/>
                                                        </xsl:if>
                                                        <xsl:if test="string-length($pages)>0">
                                                                <xsl:if test="string-length($lead)>0">
                                                                        <xsl:text>, </xsl:text>
                                                                </xsl:if>
                                                                <xsl:value-of select="$pagesPrefix"/><xsl:text> </xsl:text><xsl:value-of select="$pages"/>
                                                        </xsl:if>
                                                        <xsl:if test="string-length($lead)=0 and string-length($yearDisplay)=0 and string-length($pages)=0">
                                                        ...
                                                        <xsl:if test="msxsl:node-set($ListPopulatedWithMain)/b:Citation/b:LastAuthor">
                                                                <xsl:text>)</xsl:text>
                                                        </xsl:if>
                  ...
                  <xsl:variable name = "source">
                    <xsl:choose>
                      <xsl:when test="b:SourceType='Book'">
                        ...
                        <xsl:variable name="sg4">
                          ...
                          <xsl:if test="string-length(b:Edition)>0">
                            ...
                            <xsl:value-of select="b:Edition"/><xsl:text> ed.</xsl:text><xsl:variable name="edStr"><xsl:value-of select="b:Edition"/><xsl:text> ed.</xsl:text></xsl:variable><xsl:call-template name="need_Dot"><xsl:with-param name="field" select="$edStr"/></xsl:call-template>
                          </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="sg5">
                          <xsl:copy-of select="$sg4"/>
                          <xsl:if test="string-length(b:City)>0 or string-length(b:Publisher)>0">
                            <xsl:if test="string-length(normalize-space($sg4))>0">
                              <xsl:text> </xsl:text>
                            </xsl:if>
                            <xsl:if test="string-length(b:City)>0"><xsl:value-of select="b:City"/><xsl:text>: </xsl:text></xsl:if><xsl:value-of select="b:Publisher"/>
                          </xsl:if>
                        </xsl:variable>
                      ...
                      <xsl:when test="b:SourceType='BookSection'">
                        ...
                        <xsl:variable name="sg4">
                          <xsl:copy-of select="$sg3"/>
                          <xsl:if test="string-length(b:BookTitle)>0 or string-length($editorLF)>0">
                            <xsl:if test="string-length(normalize-space($sg3))>0">
                              <xsl:text> </xsl:text>
                            </xsl:if>
                            <xsl:text>in </xsl:text><xsl:if test="string-length($editorLF)>0"><xsl:call-template name="secBy"><xsl:with-param name="role" select="&quot;Editor&quot;"/></xsl:call-template><xsl:choose><xsl:when test="$cEditors=1"><xsl:text> (ed.) </xsl:text></xsl:when><xsl:otherwise><xsl:text> (eds.) </xsl:text></xsl:otherwise></xsl:choose></xsl:if><xsl:call-template name="ApplyItalicTitleNS"><xsl:with-param name="data"><xsl:value-of select="b:BookTitle"/></xsl:with-param></xsl:call-template>
                          </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="sg5">
                          <xsl:copy-of select="$sg4"/>
                        ...
                        <xsl:variable name="sg6">
                          <xsl:copy-of select="$sg5"/>
                          <xsl:if test="string-length(normalize-space($ppPages))>0">
                            <xsl:if test="string-length(normalize-space($sg5))>0">
                              <xsl:text>, </xsl:text>
                            </xsl:if>
                            <xsl:apply-templates select="msxsl:node-set($ppPages)" mode="outputHtml"/>
                          </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="rf">
                          <xsl:if test="string-length($doi)=0">
                      ...
                      <xsl:when test="b:SourceType='JournalArticle'">
                        ...
                        <xsl:variable name="sg5">
                          ...
                          <xsl:if test="string-length($volume)>0">
                            ...
                            <xsl:value-of select="$volume"/><xsl:if test="string-length(b:Issue)>0"><xsl:text>(</xsl:text><xsl:value-of select="b:Issue"/><xsl:text>)</xsl:text></xsl:if>
                          </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="sg6">
                          <xsl:copy-of select="$sg5"/>
                      ...
                      <xsl:when test="b:SourceType='Patent'">
                        ...
                        <xsl:variable name="sg3">
                          ...
                          <xsl:if test="string-length(b:Title)>0">
                            ...
                            <xsl:call-template name="templ_prop_OpenQuote"/><xsl:value-of select="b:Title"/><xsl:call-template name="templ_prop_CloseQuote"/>
                          </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="sg4">
                          <xsl:copy-of select="$sg3"/>
  ...
  <xsl:template name="templateRDAFU">
    <xsl:if test="string-length(b:URL)>0">
      <xsl:text>Available at: </xsl:text><xsl:value-of select="b:URL"/>
    </xsl:if>
