...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"	xmlns:b="http://schemas.openxmlformats.org/officeDocument/2006/bibliography" xmlns:t="http://www.microsoft.com/temp">
  ...
  <xsl:template match="/">
    ...
    <xsl:choose>
      ...
      <xsl:when test="b:OfficeStyleKey">
        <xsl:text>VancouverSuperscript</xsl:text>
      </xsl:when>
                                ...
                                <html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40">
                                        ...
                                        <body>
                                                ...
                                                <xsl:attribute name="dir">
                                                        ...
                                                        <sup class="MsoBibliography">
                                                                <xsl:choose>
                                                                        <xsl:when test="string-length($refOrder)>0">
                                                        ...
                                                        <xsl:if test="string-length($pages)>0">
                                                                <xsl:text>(</xsl:text>
                                                                <xsl:value-of select="$pagesPrefix"/>
                                                                <xsl:text> </xsl:text>
            ...
            <xsl:for-each select="msxsl:node-set($sList)/b:Bibliography/b:Source">
              ...
              <xsl:element name="p">
                  ...
                  <xsl:variable name = "source">
                    <xsl:choose>
                      <xsl:when test="b:SourceType='Book'">
                        ...
                        <xsl:variable name="sg2">
                          ...
                          <xsl:if test="string-length($title)>0">
                            ...
                            <xsl:value-of select="$title"/><xsl:call-template name="need_Dot"><xsl:with-param name="field" select="$title"/></xsl:call-template>
                          </xsl:if>
                        </xsl:variable>
                      ...
                      <xsl:when test="b:SourceType='JournalArticle'">
                        ...
                        <xsl:variable name="sg4">
                          <xsl:copy-of select="$sg3"/>
                          <xsl:if test="string-length($year)>0 or string-length(b:Volume)>0 or string-length(b:Pages)>0">
                            <xsl:if test="string-length(normalize-space($sg3))>0">
                              <xsl:text> </xsl:text>
                            </xsl:if>
                            <xsl:value-of select="$year"/><xsl:if test="string-length(b:Volume)>0"><xsl:text>;</xsl:text><xsl:value-of select="b:Volume"/><xsl:if test="string-length(b:Issue)>0"><xsl:text>(</xsl:text><xsl:value-of select="b:Issue"/><xsl:text>)</xsl:text></xsl:if></xsl:if><xsl:if test="string-length(b:Pages)>0"><xsl:text>:</xsl:text><xsl:value-of select="b:Pages"/></xsl:if>
                          </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="rf">
                          <xsl:if test="string-length($doi)=0">
                      ...
                      <xsl:when test="b:SourceType='DocumentFromInternetSite'">
                        ...
                        <xsl:variable name="sg4">
                          <xsl:copy-of select="$sg3"/>
                          <xsl:if test="string-length(b:URL)=0 and string-length($title)>0">
                            <xsl:if test="string-length(normalize-space($sg3))>0">
                              <xsl:text> </xsl:text>
                            </xsl:if>
                            <xsl:call-template name="need_Dot"><xsl:with-param name="field" select="$title"/></xsl:call-template>
                          </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="sg5">
                          <xsl:copy-of select="$sg4"/>
                ...
                <xsl:if test="string-length($source)>0">
                  <xsl:value-of select="b:RefOrder"/><xsl:text>. </xsl:text>
  ...
  <xsl:template name="templateRDAFU">
    <xsl:if test="string-length(b:YearAccessed)>0 or string-length(b:MonthAccessed)>0 or string-length(b:DayAccessed)>0">
      <xsl:text>[cited </xsl:text><xsl:call-template name="formatDateAccessed"/><xsl:text>]. </xsl:text>
    </xsl:if>
    <xsl:if test="string-length(b:URL)>0">
      <xsl:text>Available from: </xsl:text><xsl:value-of select="b:URL"/>
    </xsl:if>
  </xsl:template>
