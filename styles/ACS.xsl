  ...
  <xsl:template match="/">
    ...
    <xsl:choose>
      ...
      <xsl:when test="b:OfficeStyleKey">
        <xsl:text>AmericanChemicalSociety</xsl:text>
      </xsl:when>
      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1033'">
            <xsl:text>American Chemical Society</xsl:text>
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>American Chemical Society</xsl:text>
          </xsl:otherwise>
                    ...
                    <xsl:choose>
                      <xsl:when test="b:SourceType='Book'">
                        ...
                        <xsl:variable name="sg3">
                          ...
                          <xsl:if test="string-length(b:Publisher)>0 or string-length(b:City)>0 or string-length($year)>0">
                            <xsl:if test="string-length(normalize-space($sg2))>0">
                              <xsl:text>; </xsl:text>
                            </xsl:if>
                            <xsl:value-of select="b:Publisher"/><xsl:if test="string-length(b:City)>0"><xsl:text>: </xsl:text><xsl:value-of select="b:City"/></xsl:if><xsl:if test="string-length($year)>0"><xsl:text>, </xsl:text><xsl:value-of select="$year"/></xsl:if>
                          </xsl:if>
                        </xsl:variable>
