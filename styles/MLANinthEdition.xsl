  ...
  <xsl:template match="/">
    ...
    <xsl:choose>
      ...
      <xsl:when test="b:OfficeStyleKey">
        <xsl:text>MLANinthEdition</xsl:text>
      </xsl:when>
      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1033'">
            <xsl:text>MLA Ninth Edition</xsl:text>
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>MLA Ninth Edition</xsl:text>
          </xsl:otherwise>
                                        ...
                                        <body>
                                                ...
                                                <xsl:variable name="author0">
                                                        ...
                                                        <xsl:if test="string-length($pages)>0">
                                                                <xsl:if test="string-length($lead)>0 and string-length($lead)>0">
                                                                        <xsl:text> </xsl:text>
                                                                </xsl:if>
                      ...
                      <xsl:when test="b:SourceType='BookSection'">
                        ...
                        <xsl:variable name="sg2">
                          ...
                          <xsl:if test="string-length($title)>0">
                            ...
                            <xsl:call-template name="templ_prop_OpenQuote"/><xsl:value-of select="$title"/><xsl:call-template name="need_Dot"><xsl:with-param name="field" select="$title"/></xsl:call-template><xsl:call-template name="templ_prop_CloseQuote"/>
                          </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="sg3">
                          <xsl:copy-of select="$sg2"/>
